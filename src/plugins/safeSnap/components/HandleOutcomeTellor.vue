...
watch(questionState, (newState, oldState) => {
  console.log(`Question state changed from ${oldState} to ${newState}`);
});

onMounted(async () => {
  await updateDetails();
  console.log(`Initial question state: ${questionState.value}`);
});
</script>
